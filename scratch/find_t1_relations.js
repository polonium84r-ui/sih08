...
async function main() {
  console.log('Searching for Jolarpettai - Erode - Coimbatore relations...');
  const q = `[out:json][timeout:30];
relation["route"="railway"](10.8,76.8,12.6,78.6);
out tags;`;
  try {
    const data = await queryOverpass(q);
    console.log(`Found ${data.elements.length} relations.`);
